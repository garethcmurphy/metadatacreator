const H5Type = require("hdf5/lib/globals.js").H5Type;
...
export class GetH5Info {
  ...
  public geth5member(id: string, name: string) {
    const member = h5lt.readDataset(id, name);
    return member;
  }

  public getInfo(fileName: string) {
    ...
    if (rootGroups.includes("entry")) {
      const entryGroup = file.openGroup("/entry/");
      const entryMembers = entryGroup.getMemberNames();
